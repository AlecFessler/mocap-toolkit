...
int init_decoder(
  ...
  if (!codec) {
    log_write(ERROR, "Could not find cuvid H.264 decoder");
    return -ENODEV;
  }

  dec->ctx = avcodec_alloc_context3(codec);
  if (!dec->ctx) {
    log_write(ERROR, "Could not allocate decoder context");
    return -ENOMEM;
  }

  dec->hw_device_ctx = av_buffer_ref(shared_hw_device_ctx);
  if (!dec->hw_device_ctx) {
    log_write(ERROR, "Failed to reference shared hw device context");
    goto cleanup;
  }

  dec->ctx->hw_device_ctx = av_buffer_ref(shared_hw_device_ctx);
  if (!dec->ctx->hw_device_ctx) {
    log_write(ERROR, "Failed to reference hw device context for codec");
    goto cleanup;
  }
  ...
  if (!dec->hw_frame || !dec->pkt) {
    log_write(ERROR, "Failed to allocate frame/packet");
    goto cleanup;
  }
...
int recv_frame(decoder* dec, void** dev_ptr) {
  int ret = avcodec_receive_frame(dec->ctx, dec->hw_frame);
  if (ret == AVERROR(EAGAIN)) {
    return EAGAIN; // need more frames
  } else if (ret == AVERROR_EOF) {
    return ENODATA; // end of stream
  } else if (ret < 0) {
    log_write(ERROR, "Error receiving frame from decoder");
    return ret;
  }
...
int flush_decoder(decoder* dec) {
  int ret = avcodec_send_packet(dec->ctx, nullptr);
  if (ret < 0) {
    log_write(ERROR, "Error flushing decoder");
    return ret;
  }
