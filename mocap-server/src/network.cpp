...
int accept_conn(int sockfd) {
  ...
  if (clientfd < 0) {
    if (errno == EWOULDBLOCK) {
      log_write(ERROR, "Accept connection timed out, no camera connected");
      return -ETIMEDOUT;
    }
    snprintf(
      logstr,
...
ssize_t recv_from_stream(int clientfd, char* buf, size_t size) {
  ...
  if (bytes == 0) {
    log_write(WARNING, "Client has disconnected");
    return 0;
  } else if (bytes < 0) {
    if (errno == EINTR)
      return -EINTR;
    if (errno == EWOULDBLOCK) {
      log_write(WARNING, "Timed out waiting for packet from client");
      return -ETIMEDOUT;
    }
