...
pub extern fn av_hwdevice_ctx_create(
    ...
    @"type": HWDeviceType,
    device: ?[*:0]const u8,
    opts: ?*Dictionary.Mutable,
    flags: c_int,
) c_int;

pub extern fn av_hwframe_transfer_data(dst: *Frame, src: *const Frame, flags: c_int) c_int;
...
pub fn create_hwdevice_context(
    /// The type of hardware device to create
    device_type: HWDeviceType,
    /// Optional device-specific string identifier
    device: ?[*:0]const u8,
    /// Optional dictionary of device-specific options
    opts: ?*Dictionary.Mutable,
...
pub fn hwframe_transfer_data(dst: *Frame, src: *const Frame, flags: c_int) Error!void {
    // Use the existing wrap function to convert FFmpeg error codes to Zig errors
    _ = try wrap(av_hwframe_transfer_data(dst, src, flags));
}
...
pub const HWDeviceType = enum(c_int) {
    NONE = 0,
    VDPAU = 1,
    CUDA = 2,
    VAAPI = 3,
